...
The key words "**MUST**", "**MUST NOT**", "**REQUIRED**", "**SHALL**", "**SHALL NOT**", "**SHOULD**", "**SHOULD NOT**", "**RECOMMENDED**", "**MAY**", and "**OPTIONAL**" in this document are to be interpreted as described in [RFC2119] and [RFC8174] when, and only when, they are written in all capital capitals.

## 2 Credential Storage

For each PaSO Credential, the Wallet stores the following artefacts together:
...
3. **Resolved resources**: all external resources resolved for this credential — including SVG templates, resources embedded within SVG documents (per [PaSO Proof SD-JWT-VC and SVG]), and any other resource fetched for credential display. For each resource, the Wallet stores:
   - the URL from which the resource was retrieved,
   - the resource content,
   - its [W3C.SRI] integrity value, if present,
   - the date and time of retrieval.

## 3 Transaction Storage

For each [OID4VP] presentation session involving PaSO transaction data, whether completed or not, the Wallet stores:

1. **The [OID4VP] Authorization Request** as received by the Wallet. If the request was signed, the original signed form is stored.
...
3. **Resolved resources**: all external resources resolved during the transaction — including SVG templates, resources embedded within SVG documents, and any other resource fetched for transaction display. For each resource, the Wallet stores:
   - the URL from which the resource was retrieved,
   - the resource content,
   - its [W3C.SRI] integrity value, if present,
   - the date and time of retrieval.
